...
export default function SearchGame({ games, setGames }) {
    ...
    return (
        <div className="container d-flex m-2">
            ...
            <div className="">
                { result.status === 'success' && <div className="alert alert-success" role="alert">{result.message}</div> }
                { result.status === 'failed' && <div className="alert alert-danger" role="alert">{result.message}</div> }
            </div>
        </div>
    );
}